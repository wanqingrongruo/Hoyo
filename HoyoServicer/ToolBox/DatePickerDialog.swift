...
class DatePickerDialog: UIView {
    ...
    typealias DatePickerCallback = (_ date: String) -> Void
    ...
    func show(_ title: String, doneButtonTitle: String = "确认", cancelButtonTitle: String = "取消", defaultDate: Date = Date(), datePickerMode: UIDatePickerMode = .dateAndTime,
              formate: String,callback: @escaping DatePickerCallback) {
        ...
        dataFormatter.dateFormat=self.format
        let dateAndTime=dataFormatter.string(from: defaultDate)
...
//    setTime2.text=dateAndTime
    ...
    func buttonTapped(_ sender: UIButton!) {
        if sender.tag == kDatePickerDialogDoneButtonTag {
                ...
                let dataFormatter=DateFormatter()
                dataFormatter.dateFormat=self.format
                let dateAndTime=dataFormatter.string(from: select)
                print(dateAndTime)
              //  setTime2.text=dateAndTime
            self.callback?(dateAndTime)
        }
    ...
    func dateFromTimeString(_ timeStr:String)->Date
    {
        ...
        inputFormatter.dateFormat=self.format
