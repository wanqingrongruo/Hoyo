...
@interface YZDisplayViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIGestureRecognizerDelegate>
...
// 为了边缘手势返回
//@property (nonatomic,assign) CGFloat myScrollContentOffset; // scrollView 的偏移量 x
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
       ...
       // [self.contentView.gestureRecognizers.requireGestureRecognizerToFail(self.navigationController.interactivePopGestureRecognizer!)
        [self.contentScrollView.panGestureRecognizer requireGestureRecognizerToFail:self.navigationController.interactivePopGestureRecognizer];
...
//**********************************************************//
//#warnning: 为了符合设计,将宽度等分了---需要更改在这里改(这里是下划线)
//***********************************************************//
    
    // 最开始不需要动画
    if (self.underLine.x == 0) {
        self.underLine.width = [UIScreen mainScreen].bounds.size.width/2.0;
...
//********************************************************
// 当滑动到最左边时再滑动就 pop 回上一个控制器 --- 在子类中重写具体实现方法

- (void)popToLastViewControllerWhenSlideLeftest{
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
   ...
   // self.myScrollContentOffset = scrollView.contentOffset.x;
//    CGFloat offsetX = scrollView.contentOffset.x ;
//    if (offsetX <= 0) {
//        [self popToLastViewControllerWhenSlideLeftest] ;
//    }
}
...
//            if (otherGestureRecognizer.state == UIGestureRecognizerStateBegan && self.myScrollContentOffset == 0) {
//                return YES;
//            }
//        }
//        
