...
class RNSubmitCell03: SubmitCell{
    ...
    // 安装 View
    @IBOutlet weak var installView: UIView! // 安装 view
    @IBOutlet weak var installViewHeightConstraint: NSLayoutConstraint! //根据情况对高度约束进行至0
    @IBOutlet weak var codeTextField: UITextField! // 合同编号 tag=109
    @IBOutlet weak var selectRentButton: UIButton! // 选择租有效期 tag=13
    @IBOutlet weak var iyTDSTextField: UITextField! // 源水 TDS tag=110
    @IBOutlet weak var ihTDSTextField: UITextField! // 活水 TDS tag=111
    @IBOutlet weak var waterpressTextField: UITextField! // 水压 tag=112
    @IBOutlet weak var openBoxTextField: UITextField! // 开箱状态 tag=113
   ...
   // var selectedIndex = 5 //  现金已支付-微信支付-无需支付按钮 索引,默认选中现金已支付
    ...
    override func awakeFromNib() {
       ...
       // let longTap01 = UILongPressGestureRecognizer(target: self, action: #selector(exchangePicture)
    }
...
extension RNSubmitCell03{
    ...
    // 选择租赁有效期
    @IBAction func selectRentTimeAction(_ sender: UIButton) {
        
        DatePickerDialog().show("选择租赁有效期", doneButtonTitle: "确认",cancelButtonTitle: "取消",datePickerMode: .date,formate : "yyyy-MM-dd ") {
            (date) -> Void in
            self.selectRentButton.setTitle("\(date)", for: UIControlState())
        }
...
extension RNSubmitCell03: UITextFieldDelegate{
    
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if textField.tag == 102 || textField.tag == 105 || textField.tag == 109  {
...
extension RNSubmitCell03: XLPhotoBrowserDatasource{
